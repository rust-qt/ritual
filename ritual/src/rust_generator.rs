        ...
        match cpp_ffi_type.conversion {
                ...
                if argument_meaning == &CppFfiArgumentMeaning::ReturnValue {
                    ...
                    match *allocation_place {
                        ...
                        ReturnValueAllocationPlace::Heap => {
                            ...
                            *base =
                                RustName::new(vec!["cpp_utils".to_string(), "CppBox".to_string()])?;
            ...
            CppTypeConversionToFfi::ReferenceToPointer => {
                match *indirection {
                    ...
                    _ => {
                        return Err(unexpected("invalid indirection for ReferenceToPointer").into())
                    }
                }
                rust_api_to_c_conversion = RustToCTypeConversion::RefToPtr;
            }
...
impl<'aa> RustGenerator<'aa> {
                            ...
                            if &info.class_type == &class_type && (info.is_signal || info.is_slot) {
                                add_to_multihash(
                                    ...
                                    RustQtReceiverDeclaration {
                                        ...
                                        arguments: method.arguments.iter().map_if_ok(
                                            |arg| -> Result<_> {
                                                Ok(complete_type(
    ...
    fn generate_rust_single_method(
        ...
        if return_type.rust_api_type.is_ref() && return_type.rust_api_type.lifetime().is_none() {
            ...
            for arg in &arguments {
                if let Some(lifetime) = arg.argument_type.rust_api_type.lifetime() {
                    return_type.rust_api_type =
                        return_type.rust_api_type.with_lifetime(lifetime.clone());
                    found = true;
                    break;
                }
            }
            if !found {
                ...
                return_type.rust_api_type =
                    return_type.rust_api_type.with_lifetime(return_lifetime);
            }
        }
    ...
    fn calculate_rust_name(
        ...
        let module_name = self.top_module_names.get(include_file).with_context(|| {
