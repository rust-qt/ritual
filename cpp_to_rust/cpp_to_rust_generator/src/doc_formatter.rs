...
pub fn rust_method_variant(
  args: &RustMethodArgumentsVariant,
  method_name: &str,
  ...
  let arg_texts = args
    ...
    .iter()
    .map(|x| {
      rust_type_to_code(&x.argument_type.rust_api_type, crate_name)
    })
    .join(", ");
  ...
  format!(
    "fn {name}({self_arg}{arg_text}) -> {return_type}",
    name = method_name,
...
pub fn type_doc(type1: &RustTypeDeclaration) -> String {
  let auto_doc = match type1.kind {
    ...
    RustTypeDeclarationKind::MethodParametersTrait {
      ...
      format!(
        "This trait represents a set of arguments accepted by [{name}]({link}) \
                      method.",
...
pub fn enum_value_doc(value: &RustEnumValue) -> String {
  ...
  if value.cpp_docs.len() > 1 {
    ...
    for cpp_doc in &value.cpp_docs {
      doc.push_str(&format!(
        "- {}{}\n",
        wrap_inline_cpp_code(
          &format!("{} = {}", cpp_doc.variant_name, value.value),
        ),
...
pub fn method_doc(method: &RustMethod) -> String {
  ...
  for doc_item in &method.variant_docs {
    if let Some(ref item_doc) = doc_item.doc {
      if item_doc.mismatched_declaration.is_some() {
        ...
        if shown_docs.iter().any(|x| if let Some(ref xx) = x.doc {
          &xx.anchor == anchor
        } else {
          false
        })
  ...
  for (doc_index, doc_item) in shown_docs.into_iter().enumerate() {
    ...
    doc.push(format!(
      "C++ method: {}",
      wrap_inline_cpp_code(&doc_item.cpp_fn)
    ));
    doc.push("\n\n".to_string());
...
pub fn doc_for_qt_builtin_receiver(
  cpp_type_name: &str,
  rust_type_name: &str,
  receiver: &RustQtReceiverDeclaration,
) -> String {
  format!(
    "Represents a built-in Qt {signal} `{cpp_type}::{cpp_method}`.\n\n\
...
pub fn doc_for_qt_builtin_receiver_method(
  cpp_type_name: &str,
  receiver: &RustQtReceiverDeclaration,
) -> String {
  format!(
    "Returns an object representing a built-in Qt {signal} `{cpp_type}::{cpp_method}`.\n\n\
