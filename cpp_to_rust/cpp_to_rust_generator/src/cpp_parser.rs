...
pub struct CppParserConfig {
  ...
  /// Path to the html log generated by the parser
  pub html_log_path: PathBuf,
...
pub fn run(
  ...
  let mut parser = CppParser {
    ...
    current_database,
    dependencies_data,
    html_logger: HtmlLogger::new(&config.html_log_path, "C++ parser log")?,
  };
  parser.current_database.invalidate_env(&parser.env);
  parser.html_logger.add_header(&["Item", "Status"]);
