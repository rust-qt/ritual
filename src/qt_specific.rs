...
pub fn fix_header_names(data: &mut CppData, headers_dir: &PathBuf) -> Result<()> {
  let map = try!(HeaderNameMap::new(headers_dir));
  ...
  for t in &mut data.template_instantiations {
    t.include_file = map.real_to_fancy(&t.include_file, Some(&t.class_name));
  }
  Ok(())
}
...
fn test_qt_fix_header_names() {
  ...
  let map = HeaderNameMap::new(&manifest_dir().with_added("test_assets").with_added("qt_headers"))
    .unwrap();
